...
- 사용자는 Todo를 생성할 때, 자신이 소유한 마스코트 중 하나를 해당 할 일에 배정할 수 있습니다.
- Todo 완료 시, 배정된 마스코트가 '기쁨' 애니메이션과 함께 경험치를 획득합니다.
...
class Todo {
  ...
  String title;
  bool isCompleted;
  String linkedMascotId; // 할 일에 연결된 마스코트 ID
  DateTime targetDate;
}
...
- 각 Todo 아이템 우측에 배정된 마스코트 아이콘 노출.
